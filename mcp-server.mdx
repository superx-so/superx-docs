...
For the tool chains behind a whole job (a weekly recap, a week of posts, a queue reshuffle, a DM campaign), see [Agent skills](/skills): one recipe per skill, each naming the tools to call in order and where the chain stops.
...
| `publish_post` | Publish a post or thread to X immediately (irreversible; requires an `idempotency_key` you choose and reuse on retries) |
| `bulk_retime_scheduled_posts` | Move up to 500 queued posts to new times in one transaction |
| `bulk_enable_auto_retweet` | Turn Auto Retweet on for up to 100 queued posts that do not already have it |
| `bulk_delete_scheduled_posts` | Delete up to 100 queued posts and refund their post quota |
...
| `create_engage_feed` | Save a new Engage feed: keywords, a public X list, or one of the user's contact lists (does not switch the feed the app has open) |
| `update_engage_feed` | Rename an Engage feed or replace what it watches (one source at a time; the feed keeps its id) |
| `delete_engage_feed` | Delete an Engage feed; if it was the open one, the first remaining feed takes over |
| `update_signal_agent` | Edit a signal agent's name, ICP, precision mode, destination list, or status |
| `add_signal_agent_signal` | Add one thing for an agent to watch: a search, an account, its followers, or a public X list |
| `remove_signal_agent_signal` | Remove one signal from an agent (leads it already found stay) |
| `set_lead_feedback` | Record the user's verdict on one lead (`fit`, `not_fit`, or null to clear); it trains the scorer |
| `list_cover_styles` | The article cover styles saved in the app, with the ids `generate_article_cover` takes as `style_id` |
| `list_datasets` | The audience collections Ask SuperX built in the app, newest first (kept for 30 days) |
| `get_dataset` | One dataset's status, counts and coverage sentence; poll it while a collection runs |
| `get_dataset_rows` | A page of a ready dataset's rows, exactly as they were collected |
| `add_dataset_to_contact_list` | Copy the people in a ready dataset into a contact list the user created |
| `lookup_x_post` | Read one public X post live: text, author, counts, media, card, quoted post (`include_quotes` for a page of quotes) |
| `get_x_post_replies` | The top replies by likes to any public post, live (up to 3 relevance-ranked pages) |
| `lookup_x_user` | Read one public X profile live by @handle |
| `get_x_user_posts` | One live timeline page (about 20) of any public account's latest posts |
| `search_inspiration_media` | Search the cross-platform media index behind the app's Inspiration > Media tab (`source_url` only; no media file URLs) |
| `get_audience` | A cursor page of the account's followers, following, repliers or reposters from SuperX's synced snapshot |
| `get_mentions` | The posts @-mentioning the account right now, each with the post it replies to (costs 3 feed fetches) |
| `collect_audience` | Collect a post's repliers, quoters or reposters, an X list's members, or the account's own posts, into a dataset |
| `search_leads` | One live keyword search for people on X now, scored against an ideal-customer profile. Recent posts only. Saves nothing |
| `research_profiles` | Turn handles, a contact list, an agent's leads or a dataset into outreach briefs saved as a dataset |
| `draft_outreach_dms` | Write one personalized message per person onto a research dataset. Text only: nothing is sent |
| `refine_dataset` | Filter a dataset by what each person wrote, into a new dataset |
| `draft_reply` | Draft ONE reply to a post, in the user's voice. Text only: nothing is posted or sent |
| `remix_post` | Rewrite a post in the user's voice, near or far from the original |
| `inline_edit` | Edit one selected piece of a post, keeping the surrounding style |
| `rephrase_text` | Apply one preset rewrite to a post (improve, hook, concise, translate, and more) |
| `fact_check` | Check a statement against a web search: true, false or unknown, with the sources |
| `predict_viral_score` | Score ONE draft 0-100 against the account's own normal post, with what helped and what hurt |
| `triage_posts` | Search recent public posts on a topic and sort up to 40 into Read, Pass or Not sure, with the answers behind each call |
| `regenerate_style_guide` | Rebuild the account's generated style guide from its recent posts (free, once an hour) |
| `scrape_product` | Re-read a saved product's page and refresh its stored details (free) |
| `suggest_keywords` | Turn an audience description into 2 or 3 keyword-watch ideas. Creates nothing |
| `expand_icp` | Turn an audience description into the rubric signal agents score people against. Creates nothing |
| `expand_icp_from_url` | Read a website and infer its audience: description, rubric and keyword ideas (free) |
| `queue_dm_campaign` | Queue direct messages to up to 100 X users. Nothing is sent by it: the SuperX app sends |
| `get_dm_campaign` | One campaign's status counts and its individual messages |
| `cancel_dm_campaign` | Cancel a campaign's unsent messages |
| `list_dm_queue` | The account's queued and recently sent direct messages |
| `get_dm_limits` | The account's DM allowances and how much of each is used (free) |
| `list_workers` | The account's Workers (scheduled AI writers set up in the app) with their voice, topic source, schedule and next run time |
| `list_worker_suggestions` | The posts the Workers wrote, newest first; `to_review` by default, the queue waiting on a person |
| `draft_worker_suggestion` | Save one Worker suggestion as a draft post. Nothing is posted to X |
| `schedule_worker_suggestion` | Save one Worker suggestion as a scheduled post at a time you give |
| `dismiss_worker_suggestion` | Drop one Worker suggestion out of the To review queue. No post is created or removed |

## Scopes

The forty-three read tools work with any API key. The write tools (`schedule_post`, `publish_post`, `bulk_retime_scheduled_posts`, `bulk_enable_auto_retweet`, `bulk_delete_scheduled_posts`, `update_scheduled_post`, `delete_scheduled_post`, `create_article`, `update_article`, `schedule_article`, `publish_article`, `generate_article_cover`, `add_contact_list_member`, `remove_contact_list_member`, `create_signal_agent`, `set_signal_agent_status`, `delete_signal_agent`, `upload_media_from_url`, `update_context`, `update_product`, `delete_product`, `update_queue_settings`, `draft_post`, `create_tag`, `update_tag`, `delete_tag`, `set_products`, `add_contact_note`, `update_contact_note`, `delete_contact_note`, `create_contact_list`, `rename_contact_list`, `delete_contact_list`, `add_contact_list_members`, `remove_contact_list_members`, `create_engage_feed`, `update_engage_feed`, `delete_engage_feed`, `update_signal_agent`, `add_signal_agent_signal`, `remove_signal_agent_signal`, `set_lead_feedback`, `add_dataset_to_contact_list`, `collect_audience`, `search_leads`, `research_profiles`, `draft_outreach_dms`, `refine_dataset`, `draft_reply`, `remix_post`, `inline_edit`, `rephrase_text`, `fact_check`, `predict_viral_score`, `triage_posts`, `regenerate_style_guide`, `scrape_product`, `queue_dm_campaign`, `cancel_dm_campaign`, `draft_worker_suggestion`, `schedule_worker_suggestion`, and `dismiss_worker_suggestion`) need a key with the **write** scope; with a read-only key they return an insufficient-scope error the model can relay. The post, article, contact, contact-list, and signal-agent write tools take an optional `account` parameter (an account id or `@handle`) and work on your main account or any account you have linked to it; omit `account` and the tool writes to your main account. `list_accounts` returns the ids and handles that parameter accepts, and `get_me` reports whether the key has the write scope at all. Accounts other people share with you are read-only for those tools, with a set of exceptions: `search_leads`, `draft_reply`, `remix_post`, `inline_edit`, `rephrase_text`, `fact_check`, `predict_viral_score` and `triage_posts` spend credits without changing the account, so they work on a shared account too. `draft_post` is not among them: it prepares the account's own style guide when needed, so a shared account returns `writes_main_account_only`. `regenerate_style_guide` is not one of them: it rewrites the account's own style guide, so a shared account refuses it, and neither are `queue_dm_campaign` and `cancel_dm_campaign`: sending on someone else's behalf is not something a share grants. `upload_media_from_url` takes no `account`: uploads belong to you rather than to one X account, so an `object_key` can be attached to a post on any of your accounts. Tags are workspace-wide, one set covering all your linked accounts, so `create_tag`, `update_tag`, and `delete_tag` take no `account` and always write on the main account. The context and queue-settings tools accept an `account` parameter for any linked or shared account. On an account shared with `editor` permission, `update_context`, `update_product`, `delete_product`, `set_products`, and `scrape_product` return an `editor_restricted` error while `update_queue_settings` works.
...
`research_profiles` costs 1 credit per profile actually researched (unresearched ones are returned) and reads X live, so more than 5 profiles run in the background: the result then carries `status: "collecting"` and you must poll `get_dataset` until it says `ready` before stating any brief count. A big refinement does the same. `search_leads` is a write tool for the same reason `draft_post` is: it spends AI credits and live requests, not because it saves anything, so a read-only key gets an insufficient-scope error; an account shared with you still works, because nothing is written to it. It counts only posts written inside `max_post_age_days` (1-90, 30 by default; use 7 for a pain point worth catching while it is fresh), returns each lead's `posted_at` and `post_age_days` freshest first within each score, and counts the older matches it skipped in `freshness.stale_skipped`, so an empty result with a count there means people do post about this, just not lately: broaden the keywords rather than widening the window on your own. The REST endpoint `POST /v1/signals/leads/search` behaves identically. `search_leads` and `research_profiles` also draw on a platform-wide fair-use ceiling for live-data actions on top of the plan's own daily caps; a 429 whose `scope` is `"platform"` is not about this account's quota, so wait and retry rather than telling the user they are out. `refine_dataset` counts against the same 10 dataset collections a day `collect_audience` does, keeps rows it cannot judge (counted as `unclear`), and only works on datasets whose rows carry text.

The writing helpers all return TEXT and write nothing: `draft_reply` drafts one reply to a post, `remix_post` rewrites a post as close to or as far from the original as asked (`closeness` 0 to 100), `inline_edit` changes one selected piece while keeping the surrounding style, `rephrase_text` applies one preset rewrite, `fact_check` checks a claim, and `predict_viral_score` scores one draft against the account's own normal post. **Nothing here is posted, scheduled or sent**, so never say a reply went out: show the draft, let the person edit it, and call `draft_post`, `schedule_post` or `publish_post` when they say so. There is still no reply-sending tool anywhere in this server. They are write tools for the same reason `draft_post` is: they spend AI credits. Unlike `draft_post`, a shared account works for these. `draft_reply` with a `post_id` reads the post live and spends one lookup from the same 300-a-day allowance the live X lookups use; pass the wording in `post` instead and it spends none. Ask the user what they want a reply to convey (`thoughts`) rather than inventing an opinion for them. A `fact_check` verdict is a model's reading of a couple of search results: report it with its `sources` and never as settled fact, and a `predict_viral_score` result compares a draft with the account's OWN recent posts: it is not a prediction of reach and it knows nothing about follower count. Iterate a draft with it until the score stops rising, show `helped` and `hurt` as they come back, and treat anything in `warnings` as a stop sign rather than something to work around: asking for replies, inviting people to connect, borrowing a template or sending readers off the platform can only push the score down.
...
| Duplicate scheduled posts | `schedule_post` has no idempotency. If a call timed out or was retried, check `get_scheduled_posts` before calling it again. `publish_post` is different: its `idempotency_key` makes a retry safe, so reuse the same key rather than checking first. |
| `idempotency_in_flight` on `publish_post` | An earlier call with that key is still publishing. Wait the number of seconds in the message, then retry the SAME key; it will never publish twice. |
| A bulk tool reports fewer changes than ids sent | The bulk tools only touch posts in the queue. Drafts, sent posts and error rows are counted as skipped, and `bulk_enable_auto_retweet` also skips posts that already have an auto retweet. |
