...
The search creates no agent and stores no leads, so keep what is needed from that one response. `--max-age-days` is the recency window (1-90, 30 by default): 7 for a pain point worth catching while it is fresh. Older matches are skipped and counted in `freshness.stale_skipped`. Costs at least 1 AI credit plus one of the plan's daily lead searches, and draws on a fair-use ceiling shared by every account. Takes about a minute.
