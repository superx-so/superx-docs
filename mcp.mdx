...
You need an API key from [app.superx.so/account?tab=developers](https://app.superx.so/account?tab=developers) and an active SuperX subscription.
...
The Developers tab in the SuperX app shows the ready-to-paste URL once, right when you create a key.
...
The keyed URL **is** your API key. Treat it like a password: share it only with the connector, keep it out of chat transcripts and screenshots, and rotate the key from the Developers tab if it ever leaks. Prefer header auth wherever the client supports it. SuperX redacts the key from its own request logs.

## Troubleshooting

| Symptom | Cause |
| --- | --- |
| `401 invalid_api_key` | Wrong, mistyped, or revoked key. Create or rotate a key in the Developers tab. |
