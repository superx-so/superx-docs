...
| `superx posts:draft` | Write post drafts in your voice from a `--brief` (`--count` 1-3, `--voice mine`, `creator` or `hybrid` with `--creator`, `--mirror` the text of a proven post to copy the shape of, `--collection`, `--instructions`); nothing is scheduled and each draft costs AI credits (`--account` picks which of your accounts to draft as; `--voice mine` is that account's own voice) |
...
| `superx posts:viral-score` | Score one draft against this account's own normal post (`--text` required; `--image`, `--video`, `--quote`, `--post-at`, `--population`) |
| `superx posts:triage <query>` | Sort recent public posts on a topic into Read, Pass or Not sure (`--days` 1-7, default 3). Judges the text, never the author |
...
| `superx workers:list` | List your Workers (the scheduled AI writers set up in the SuperX app) with their schedules and next run times |
| `superx workers:suggestions` | List the posts your Workers wrote, newest first (`--status to_review` default, or `drafted`/`scheduled`/`dismissed`/`all`; `--worker <id>`, `--limit`, `--page`) |
| `superx workers:draft <id>` | Save one Worker suggestion as a draft; the id comes from `workers:suggestions` |
| `superx workers:schedule <id>` | Schedule one Worker suggestion (`--at` UTC ISO-8601 required); it posts at that time like any queued post |
| `superx workers:dismiss <id>` | Drop one Worker suggestion out of the To review list (no post is created or removed) |
...
Writing helpers: `engage:reply-draft`, `posts:remix`, `tools:inline-edit`, `tools:rephrase`, `tools:factcheck` and `posts:viral-score` all return TEXT or a score and post NOTHING. `engage:reply-draft` writes one reply for a person to review and post; there is no reply-sending command anywhere in the CLI, so never report a reply as sent. Give it exactly one of `--post <id>` (the post is read live, which costs one live lookup on top of the credit) or `--text`, and use `--thoughts` for what the person actually wants to say rather than inventing an opinion for them. `posts:remix` needs `--closeness` 0-100: 0 keeps only the idea, 100 stays very close to the wording. A `tools:factcheck` verdict is a model's reading of a couple of search results, so report it with its sources and never as settled fact, and a `posts:viral-score` result compares a draft with the account's OWN recent posts (its originals from the past 90 days, minus the last 3 days, whose numbers are still settling) rather than predicting reach: rewrite what `hurt` names, score again, and read anything in `warnings` as a stop sign, because those things can only push the score down. Each of these costs measured AI credits: typically 1, and 2 for a remix or a reply draft.

`posts:triage "<query>"` is the one command here that reads other people's posts. It searches recent public posts on a topic (`--days` 1-7, default 3) and sorts up to 40 of them into `lane` `read`, `unsure` or `pass`, each with a `pct`, a `kind` and the `answers` behind the call, so you can say why a post landed where it did from its own numbers. `pct` is how CLEAR the call was and NOT how good the post is: on `read` and `pass` it runs 50 to 99, so a `pass` at 99 is a post confidently not worth reading, and only on `unsure` is it the raw worth-reading score. Always quote it with its lane, and never rank posts by it across lanes. What is judged is the TEXT of a post, and the model never sees who wrote it, so report it as a read on the writing rather than a rating of a person. The search asks for original posts, English only, with a floor of 30 likes; replies and reposts never come back, and none of those is a flag you can change, so a small or brand-new topic can come back empty. The floor is what the search asks for rather than a promise about `counts.likes`: a few posts do come back under it. It costs a flat 2 credits a run, spends one live lookup for a single-word query and two for a multi-word one, and carries a per-day cap per plan. Nothing is posted, saved or sent.
...
With the CLI already installed, `superx skills:install` does the same job from the package itself: it puts the skill in `~/.claude/skills/superx` and `~/.agents/skills/superx` (`--target claude|agents|openclaw|all`, `--project` for the current directory, `--copy` instead of a symlink). `superx skills:list` prints every bundled skill as JSON. Nothing is written on `npm install`.

The skill teaches the agent the full workflow (check auth, read analytics and contacts, create drafts, verify) and ships with a growth strategy guide, a distilled SuperX method the agent follows when deciding what and when to post.

It also ships with the goal-shaped skills documented on [Agent skills](/skills), one folder each: every one gives the command chain for a named SuperX skill, the matching MCP tools, and the point where the agent hands back to a person.
