...
It also ships with `PLAYBOOKS.md`, the 29 goal-shaped recipes documented on [Agent playbooks](/playbooks): each one gives the command chain for a named SuperX skill, the matching MCP tools, and the point where the agent hands back to a person.
