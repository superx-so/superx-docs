...
Create an API key in the SuperX app at [app.superx.so/account?tab=developers](https://app.superx.so/account?tab=developers), then:
...
| `superx scheduled:create` | Create a draft (no `--at`) or scheduled post; repeat `--part` for a thread; `--media`/`--alt-text` attach images; `--title`, `--scratchpad`, `--tag` add organizer fields |
| `superx scheduled:update <id>` | Edit a draft or scheduled post; only the flags you pass change (`--status scheduled` promotes; text replacement without `--media` removes the post's images) |
