...
| 400 | `invalid_source` | `POST /v1/datasets` was sent a `source` it does not accept. Research briefs are built in the SuperX app only |
...
| 404 | `post_not_found` | No public post with that id on the live lookups (`GET /v1/x/posts/{id}` and its `/replies`): deleted, protected, or the wrong id. The upstream read reports a missing post and a failed read identically, so a transient failure can surface here too; retry once before concluding the post is gone |
...
| 409 | `collection_in_progress` | Another background audience collection is still running for this account. Poll it with `GET /v1/datasets/{id}` and start the next one when it finishes |
...
| 429 | `collection_quota_exceeded` | Today's audience collections are used up (10 per account per day, shared with the collections Ask SuperX runs in the app). Carries `retry_after`, `limit` and `reset_at`, plus a `Retry-After` header; the counter resets at midnight UTC |
