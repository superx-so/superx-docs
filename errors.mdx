...
| 429 | `ai_action_limited` | A daily count limit for this AI action is used up. Carries `scope`, `retry_after`, `reset_at` and a `Retry-After` header; counters reset at midnight UTC. `scope: "account"` is your plan's own daily cap for the action; `scope: "platform"` is the platform-wide fair-use ceiling on live-data actions, shared with every SuperX account, so your own allowance is untouched and the fix is to wait and retry |
