...
- The same gate covers the contact notes endpoints (`GET`/`POST /v1/contacts/{id}/notes`, `PATCH`/`DELETE /v1/contacts/{id}/notes/{noteId}`): notes exist only for known contacts, so an id outside that set returns 404 `contact_not_found`
...
- `get_contact` and the four contact-note tools follow the same rule and their descriptions now say so. Pass ids from `get_top_contacts`, `get_contact_list_members` or `get_signal_leads`

**CLI**

- `superx contacts:get <id>` and `superx contacts:notes*` inherit the same 404
