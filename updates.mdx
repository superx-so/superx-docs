...
- `PATCH /v1/scheduled-posts/{id}`: edit a draft or scheduled post. Only the fields you send change; text edits, new times, title/scratchpad, tag replacement, and explicit `status` transitions (`draft` <-> `scheduled`, riding the normal quota deduct/refund). A `scheduled_for` alone never promotes a draft.
...
- API keys with read/write scopes, managed from Account > Developers in the SuperX app
