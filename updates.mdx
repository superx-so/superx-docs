...
- `refresh=true` still spends one enrichment unit before the contact is resolved, so a refresh on an unknown id is charged and then returns 404

**MCP**

- `get_contact` follows the same rule and its description now says so. Pass ids from `get_top_contacts`, `get_contact_list_members` or `get_signal_leads`

**CLI**

- `superx contacts:get <id>` inherits the same 404
