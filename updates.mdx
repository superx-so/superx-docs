...
## 2026-09-19: Triage a topic into Read, Pass or Not sure

Finding what is worth reading on a topic meant scrolling it yourself. One call now searches recent public posts and sorts them into three lanes, with the yes/no answers behind every call.

**API**

- `POST /v1/posts/triage` takes `query` (2-80 characters) and optional `max_age_days` (1-7, default 3), searches recent public posts and returns up to 40 of them sorted into `lane` `read`, `pass` or `unsure`. Each post carries `pct`, a `kind` (`insight`, `story`, `data`, `progress`, `news`, `question`, `intro`, `launch`, `joke`, `opinion`) and `answers`: specific and informative, says something new, engagement bait, a plug, a platitude, and room to add something in a reply
- **`pct` is how clear the call was, not how good the post is.** On `read` and `pass` it runs 50 to 99, measured away from the threshold in whichever direction the lane went, so a post that is confidently not worth reading is a `pass` at 99. On `unsure` it is the raw worth-reading score. Read it together with `lane` and never rank across lanes by it
- The searches ask for original posts, English only, with a floor of 30 likes, and never return replies or reposts. The like floor is the one the lane thresholds were tuned against, so it is fixed rather than a parameter, and it is what the search asks for rather than a guarantee: a few posts come back under it
- **It judges the text.** No author, handle or follower count is in what the model reads, so a result is a call on the writing and never a rating of a person. The author block comes back for display only
- A plain multi-word `query` is also searched as an exact phrase; a query that already carries quotes, a field lookup, `OR` / `AND` or an exclusion is searched once, as written. `posts_searched` says how many distinct posts the searches found before the 40-post cap, and `receipt` reports posts judged, questions answered, milliseconds and model spend
- A flat 2 credits a run, with a per-day cap of 20 on trial, 60 on pro, 150 on advanced and 300 on ultra. The searches spend your own live X allowance, the one shared with Ask SuperX in the app: 1 lookup for a single-word query, 2 for a multi-word one, out of 300 a day, which is what binds first on the higher plans. A run that finds nothing charges no credits and still counts against the daily cap. Nothing is posted, saved or sent

**MCP**

- `triage_posts`, with the same fields as the endpoint. Reconnect the server to pick it up

**CLI**

- `superx posts:triage "coding agents"` (`--days` 1-7, `--account`)

**Skills**

- New skill Worth a Reply: it reads a topic and hands back the posts worth replying to, ordered by how much room there is to add something in a reply, each with its link, one line on why and the angle a reply could take. Runs in Ask, MCP and the CLI, and drafts nothing until you pick a post. Skills are now 32

## 2026-09-18: Agent skills, one folder each

The same recipes used to live under three names in three places, and they had drifted: the agent package said "playbooks", the app said "skills", and the two sets were not even the same 29. There is now one set of 30 skills, each a folder in the [superx-agent](https://github.com/superx-so/superx-agent) repo, and everything else is generated from it.

**Docs**

- New page [Agent skills](/skills), generated from those folders: 30 recipes grouped by the same eight categories the app uses, each with its CLI chain, its MCP tool chain and where the chain stops. It replaces the old Agent playbooks page, and `/playbooks` redirects here

**Skills**

- 30 skills, up from 29 recipes and 29 app cards that were not the same 29. Worker Output Review now has an app card and Find Your Story now has a command chain, so every skill ships both
- New skill Viral Score Loop: score a draft against your own recent posts, rewrite one thing at a time keyed on what hurt, rescore, and stop when the score stops rising or a warning fires. Runs in Ask, MCP and the CLI. Skills are now 31
- Instant Lead Hunt carries `signals:search --offer` and `--max-age-days`, and says what a hunt costs instead of calling it free. Viral Format Remix says 3 AI credits per draft. Lead Review says your fit or not-fit verdicts are recorded and train the scorer

**CLI**

- `superx skills:list` prints every bundled skill as JSON, and `superx skills:install` puts the skill where your agent looks for it (`~/.claude/skills` and `~/.agents/skills` by default, `--target claude|agents|openclaw|all`, `--project`, `--copy`). `npx skills add superx-so/superx-agent` still works

## 2026-09-18: Viral Score, and algorithm-predict is gone

Scoring a draft used to mean comparing two versions on a model's opinion. The new score compares ONE draft with the account's OWN recent posts, using weights fit on 9,481 real posts from 214 accounts, and says what helped and what hurt in plain English.

**API**

- `POST /v1/posts/viral-score` scores one draft from 0 to 100 against the account's own normal post: `text` (1-4000) plus optional `has_image`, `has_video`, `is_quote`, `post_at` and `baseline` (`account` by default, or `population`). It returns `counters` for likes, replies, reposts+quotes and views (a median multiple of that account's own median post, with `p75`, `p90` and `expected`), `helped` and `hurt` with the feature that moved the score and what it did, `warnings`, and `accuracy` with the measured figure to quote
- It compares a draft with the account's own posts: it is not a reach prediction and it knows nothing about follower count. `reposts_and_quotes` and `views` come back `confidence: "low"`, because text says much less about those
- `warnings` name what the score never rewards: asking for replies as the point of the post, inviting people to connect, a structure borrowed from other accounts, and sending readers off the platform. Those can only push a score down, so rewrite rather than work around them
- The baseline is the account's last originals from the past 90 days, leaving out anything posted in the last 3 days because those numbers are still settling. Under 10 usable posts and `baseline.kind` is `population`, which scores against the average training post and returns `expected: null`
- Cheap on purpose so drafts can be iterated: most calls charge no credits, one in ten charges 1, and the first call for an account charges 1 more while its baseline is built. The per-day cap is 50 on trial, 300 on pro, 1,000 on advanced and 2,000 on ultra
- **Removed:** `POST /v1/tools/algorithm-predict`. It scored two versions on a model's opinion and was never measured against real posts

**MCP**

- `predict_viral_score` replaces `predict_algorithm`, with the same fields as the endpoint. Reconnect the server to pick it up

**CLI**

- `superx posts:viral-score --text "..."` (`--image`, `--video`, `--quote`, `--post-at`, `--population`) replaces `superx tools:predict`

## 2026-09-14: Lead search only returns recent posts

A live lead search used to reach back years to fill a page, so people whose matching post was months or years old came back looking like fresh leads. Every lead now comes from a post inside a recency window you control, and the leads are ordered freshest first within each score.

**API**

- `POST /v1/signals/leads/search` takes `max_post_age_days` (1-90, 30 by default). A matching post older than the window is skipped rather than returned; 91 or 0 is a `400 invalid_parameter`
- Each lead's `provenance` gains `posted_at` (when the matched post was written), `post_age_days`, and `tweet_id` (the matched post's id, named as it is on stored signal leads, for the endpoints that take a post id)
- `data.freshness` reports `max_post_age_days`, `posts_in_window`, `stale_skipped` and `oldest_skipped_days`. An empty `leads` with `stale_skipped` above 0 means people do post about this, just not inside the window: broaden the keywords, or raise the window when you want people active over a longer stretch rather than a fresh pain point
- Narrow searches usually spend fewer live requests now, because paging stops once a page crosses the window

**MCP**

- `search_leads` takes `max_post_age_days` and returns the same `freshness` block and per-lead post age. Reconnect the server to pick the parameter up

**CLI**

- `signals:search --max-age-days <1-90>`

**Docs**

- The [Agent playbook](/playbooks) "Instant Lead Hunt" and the [MCP server](/mcp-server) page cover the window
