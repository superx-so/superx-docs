...
## 2026-09-10: Reply drafts, remix and the composer tools

The writing helpers from the SuperX app composer are now on the API, MCP and the CLI. Every one of them returns TEXT and writes nothing: nothing here is posted, scheduled or sent.
...
- Six new tools: `draft_reply`, `remix_post`, `inline_edit`, `rephrase_text`, `fact_check` and `predict_algorithm`. All six need a key with the write scope, because all six spend AI credits. 89 tools in total
